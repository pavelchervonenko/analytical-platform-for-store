...
public class LlmAnalysisJobWorker {

    private static final Logger LOGGER = LoggerFactory.getLogger(
            LlmAnalysisJobWorker.class
    );

    private final String workerId = UUID.randomUUID().toString();
    private final LlmProviderCallJobRunner runner;
    private final LlmAnalysisWorkerProperties properties;

    public LlmAnalysisJobWorker(
            LlmProviderCallJobRunner runner,
    ...
    @Scheduled(
            ...
            scheduler = LlmAnalysisWorkerSchedulingConfiguration.LLM_PROVIDER_CALL_SCHEDULER
    )
    public void processNext() {
        try {
            ...
            LOGGER.error(
                    "LLM provider worker step failed; failure_type={}",
    ...
    @Scheduled(
            ...
            scheduler = LlmAnalysisWorkerSchedulingConfiguration
                    .LLM_PROVIDER_HEARTBEAT_SCHEDULER
    )
    public void heartbeat() {
        try {
            ...
            LOGGER.error(
                    "LLM provider worker heartbeat failed; failure_type={}",
                    exception.getClass().getSimpleName()
            );
        }
    }
