...
@Component
public class LlmPublicationClaimStore {

    private final JdbcTemplate jdbcTemplate;
    private final LlmAnalysisJobStore jobStore;

    public LlmPublicationClaimStore(
    ...
    public Optional<LlmAnalysisJob> claimNext(
        ...
        List<ClaimTarget> candidates = jdbcTemplate.query(
                ...
                WHERE job.status = 'WAITING_RETRY'
                  ...
                  AND NOT EXISTS (
                      SELECT 1 FROM llm_interpretations interpretation
                      WHERE interpretation.analysis_job_id = job.id
                  )
