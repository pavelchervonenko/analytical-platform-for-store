...
@ConditionalOnProperty(
        ...
        name = {"enabled", "order-return-enabled"},
        havingValue = "true"
)
class LiveSkladOrderReturnWebhookWorker {

    private static final Logger LOGGER = LoggerFactory.getLogger(
            LiveSkladOrderReturnWebhookWorker.class
    );
    ...
    LiveSkladOrderReturnWebhookWorker(
    ...
    void processNext() {
        ...
        Optional<LiveSkladWebhookClaim> candidate = store.claimNextOrderReturn(
                workerId,
                now,
        ...
        try {
            sourceDocumentId = sourceDocumentId(claim.payload());
        ...
        try {
            store.recordSourceDocument(
                    claim.id(), workerId, sourceDocumentId
            );
            orderSyncService.synchronizeWebhookOrder(sourceDocumentId);
    ...
    private void handleFailure(
        ...
        String summary = "Order-return webhook processing failed: " + code;
        ...
        LOGGER.warn(
                "LiveSklad order-return webhook event {} failed with {}; "
    ...
    private boolean isRetryable(Throwable failure) {
        ...
        if (find(failure, LiveSkladRateLimitException.class) != null
                ...
                || find(failure, LiveSkladOrderChangedException.class) != null
    ...
    private String failureCode(Throwable failure) {
        ...
        if (find(failure, LiveSkladOrderChangedException.class) != null) {
            return "LIVESKLAD_ORDER_CHANGED";
        }
        if (find(failure, TransientDataAccessException.class) != null) {
            return "TRANSIENT_DATABASE";
        }
        if (find(failure, InvalidRequestException.class) != null) {
            return "ORDER_RECONCILIATION_REJECTED";
        }
