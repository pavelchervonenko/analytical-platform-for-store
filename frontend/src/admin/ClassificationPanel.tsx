...
export function ClassificationPanel() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => { setError(null); setReason(""); setSelected({}); await Promise.all([queryClient.invalidateQueries({ queryKey: ["stores", selectedStore.id, "payroll"] }), queryClient.invalidateQueries({ queryKey: queryKeys.periodQuality(selectedStore.id, month, asOfDate) })]); },
  ...
  return <div className="classification-layout">
    <section className="panel classification-main"><div className="panel__heading"><div><p className="eyebrow">{selectedStore.name}, {month}</p><h2>Неразмеченные товары</h2></div><span>{products.length}</span></div>
      {products.length === 0 ? <div className="panel-empty"><CheckCircle2 size={28} /><strong>Классификация завершена</strong><p>Все товары расчетного месяца имеют категорию для расчета зарплаты.</p></div> : <form onSubmit={submit}><div className="classification-list">{products.map((product) => <article key={product.productId}><div><strong>{product.productName}</strong><small>{formatNumber(product.netQuantity)} ед., {formatMoney(product.netRevenue)}, продажи {product.firstSaleDate}—{product.lastSaleDate}</small>{product.suggestionReason && <p>Подсказка: {product.suggestionReason}</p>}</div><label><span className="sr-only">Категория товара {product.productName}</span><select value={selected[product.productId] ?? ""} onChange={(event) => setSelected((current) => ({ ...current, [product.productId]: event.target.value as PayrollCategory | "" }))}><option value="">Не выбрана</option>{categories.map((category) => <option key={category.value} value={category.value}>{category.label}</option>)}</select></label></article>)}</div><label className="field classification-reason"><span>Причина изменения</span><textarea value={reason} required maxLength={500} onChange={(event) => setReason(event.target.value)} placeholder="Например: первичная классификация товаров за месяц" /></label>{error && <p className="form-error">{error}</p>}<footer><small>Будет назначено: {assignments.length} из {products.length}. Все изменения будут сохранены вместе.</small><button className="button button--primary" disabled={assignments.length === 0 || !reason.trim() || mutation.isPending}>{mutation.isPending ? "Применяем…" : "Применить выбранные категории"}</button></footer></form>}
    </section>
    <aside className="panel classification-readiness"><span className="context-icon"><PackageSearch /></span><p className="eyebrow">Готовность зарплаты</p><h2>{readiness.status === "READY" ? "Готово" : readiness.status === "NEEDS_CORRECTION" ? "Нужны исправления" : "Расчет заблокирован"}</h2><dl><div><dt>Неразмечено</dt><dd>{readiness.unmappedItemCount}</dd></div><div><dt>Без себестоимости</dt><dd>{readiness.missingCostItemCount}</dd></div><div><dt>Дней без смен</dt><dd>{readiness.daysWithoutShift}</dd></div></dl>{readiness.missingCostItemCount > 0 && <p className="admin-form-note admin-form-note--warning"><TriangleAlert />Себестоимость нельзя исправить здесь. Проверьте данные в учетной системе.</p>}<small>Категория действует с первого числа выбранного месяца и не переписывает прошлые снимки.</small></aside>
  </div>;
}
