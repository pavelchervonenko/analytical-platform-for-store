...
export function CategoryImportPanel() {
  ...
  const submit = (event: FormEvent) => {
    ...
    if (window.confirm(`Импортировать ${parsed.assignments.length} назначений атомарно?`)) {
      setResult(null);
      mutation.mutate();
    }
  };
  ...
  return <div className="category-import-layout">
    <section className="panel category-import-main">
      <div className="panel__heading"><div><p className="eyebrow">Bootstrap-операция</p><h2>Импорт справочника категорий</h2></div><FileJson /></div>
      <p className="category-import-lead">Загрузите согласованный JSON-массив. Браузер проверит структуру и дубликаты до отправки; backend применит весь пакет в одной транзакции.</p>
      <form className="admin-form category-import-form" onSubmit={submit}>
        ...
        <label className="field category-import-editor"><span>Назначения · JSON-массив</span><textarea value={source} spellCheck={false} maxLength={5_000_000} onChange={(event) => { setSource(event.target.value); setResult(null); }} /></label>
