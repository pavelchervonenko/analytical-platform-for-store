  ...
  return <>
    ...
    <section className="panel admin-user-list">
      ...
      {usersQuery.data.items.map((user) => <article key={user.id}>
        ...
        <div><strong>{user.lastLoginAt ? new Date(user.lastLoginAt).toLocaleDateString("ru-RU") : "Не входил"}</strong></div>
