...
export function UsersPanel() {
  ...
  const usersQuery = useQuery({ queryKey: adminKeys.users, queryFn: getAdminUsers });
  ...
  return <>
    <section className="admin-section-heading"><div><p className="eyebrow">Учетные записи и полномочия</p><h2>Пользователи</h2><p>Роль определяет административные права, назначения — доступ руководителя к магазинам.</p></div><button className="button button--primary" type="button" onClick={() => open("create")}><Plus size={16} />Создать пользователя</button></section>
    <section className="panel admin-user-list">
      ...
      {usersQuery.data.map((user) => <article key={user.id}>
        ...
        <div><span className={`status status--${user.active ? "success" : "warning"}`}>{user.active ? "Активен" : "Отключен"}</span><small>{user.role === "ADMIN" ? "Администратор" : "Руководитель"}{user.passwordChangeRequired ? " · временный пароль" : ""}</small></div>
