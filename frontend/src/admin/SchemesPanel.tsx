...
export function SchemesPanel() {
  ...
  const ratingQuery = useQuery({ queryKey: adminKeys.ratingSchemes, queryFn: getRatingSchemes });
  const payrollQuery = useQuery({ queryKey: adminKeys.payrollSchemes, queryFn: getPayrollSchemes });
  const [ratingError, setRatingError] = useState<string | null>(null);
  const [payrollError, setPayrollError] = useState<string | null>(null);
  const ratingMutation = useMutation({ mutationFn: createRatingScheme, onSuccess: async () => { setRatingError(null); await queryClient.invalidateQueries({ queryKey: adminKeys.ratingSchemes }); }, onError: (error) => setRatingError(errorText(error)) });
  const payrollMutation = useMutation({ mutationFn: createPayrollScheme, onSuccess: async () => { setPayrollError(null); await queryClient.invalidateQueries({ queryKey: adminKeys.payrollSchemes }); }, onError: (error) => setPayrollError(errorText(error)) });
