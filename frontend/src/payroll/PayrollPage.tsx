...
function payrollErrorMessage(error: unknown): string {
  if (isApiClientError(error)) {
    if (error.code === "PAYROLL_SOURCE_DATA_CHANGED") return "Исходные данные изменились. Расчёт обновлён для проверки — выполните явный перерасчёт.";
    if (error.status === 409) return "Ревизия уже изменилась. Данные перечитаны; проверьте актуальную версию и повторите действие.";
    if (error.status === 403) return "Недостаточно прав для изменения этого расчёта.";
...
function statusLabel(status: string): string {
  ...
  if (status === "APPROVED") return "Расчёт утверждён";
...
function Workflow({ canCalculate, run }: { canCalculate: boolean; run: PayrollRunDetail["run"] | null }) {
  ...
  const steps = [
    ...
    { title: "Расчёт", caption: run ? `Ревизия №${run.revision}` : "Не создан", complete: Boolean(run), active: run?.status === "CALCULATED" },
  ...
  return <ol className="payroll-workflow" aria-label="Этапы расчёта зарплаты">{steps.map((step, index) => <li className={`${step.complete ? "complete" : ""} ${step.active ? "active" : ""}`} key={step.title}><span>{step.complete ? <Check /> : index + 1}</span><div><strong>{step.title}</strong><small>{step.caption}</small></div></li>)}</ol>;
}
...
export function PayrollPage() {
  ...
  const calculateMutation = useMutation({ mutationFn: (reason?: string) => calculatePayroll(storeId, month, reason), onSuccess: onSuccess("Расчёт сохранён. Актуальная ревизия загружена."), onError: onConflict });
  ...
  const approveMutation = useMutation({ mutationFn: () => approvePayroll(storeId, latestQuery.data!.run.id, latestQuery.data!.run.version), onSuccess: onSuccess("Расчёт утверждён и больше не редактируется."), onError: onConflict });
  const paidMutation = useMutation({ mutationFn: () => markPayrollPaid(storeId, latestQuery.data!.run.id, latestQuery.data!.run.version), onSuccess: onSuccess("Выплата отмечена. Месячный workflow завершён."), onError: onConflict });
  ...
  return <div className="payroll-page"><header className="page-heading payroll-heading"><div><p className="eyebrow">{selectedStore.name}</p><h1>Зарплата и аудит</h1><p>Расчёт, проверка и фиксация выплат за {formatMonth(month)}.</p></div>{detail && <div className="payroll-heading-status"><small>Текущая ревизия</small><strong>№{detail.run.revision} · {statusLabel(detail.run.status)}</strong><span className={`status ${detail.run.freshness.status === "CURRENT" ? "status--success" : "status--warning"}`}>{detail.run.freshness.status === "CURRENT" ? "Актуальна" : "Нужен перерасчёт"}</span></div>}</header><Workflow canCalculate={readiness.canCalculate} run={detail?.run ?? null} /><ReadinessBanner readiness={readiness} run={detail?.run ?? null} search={baseSearch} isAdmin={user?.role === "ADMIN"} />{notice && <section className="payroll-notice" role="status"><CheckCircle2 /><span>{notice}</span><button type="button" onClick={() => setNotice(null)} aria-label="Скрыть уведомление"><X /></button></section>}

    <nav className="payroll-tabs" aria-label="Разделы зарплаты"><button className={section === "statement" ? "active" : ""} type="button" onClick={() => selectSection("statement")}><ReceiptText />Ведомость<span>{detail ? `ревизия №${detail.run.revision}` : "предпросмотр"}</span></button><button className={section === "days" ? "active" : ""} type="button" onClick={() => selectSection("days")}><CalendarDays />Дневные фонды<span>{detail?.dailyPools.length ?? preview?.actualScenario.days.length ?? 0} дней</span></button><button className={section === "audit" ? "active" : ""} type="button" onClick={() => selectSection("audit")}><History />Аудит<span>{detail ? `${detail.events.length} событий` : "после расчёта"}</span></button></nav>
