...
import { hasUserFeature, type PayrollAdjustment, type PayrollAdjustmentInput, type PayrollRunDetail } from "../api/contracts";
...
import { formatPayrollCheckCount, payrollCategoryLabel, payrollErrorMessage, payrollReadinessCheckCount, staleReasonLabel } from "./payroll-ui";
import "./styles.css";
...
function Workflow({ canCalculate, run }: { canCalculate: boolean; run: PayrollRunDetail["run"] | null }) {
  ...
  const steps = [
    ...
    { title: "Проверка", caption: canCalculate ? "Доступен" : "Ожидает", complete: Boolean(run), active: canCalculate && !run },
    { title: "Расчет", caption: run ? `Версия №${run.revision}` : "Не создан", complete: Boolean(run), active: run?.status === "CALCULATED" },
...
export function ReadinessBanner({ readiness, run, search, isAdmin, canOpenPlan, canOpenShifts }: { readiness: Awaited<ReturnType<typeof getPayrollReadiness>>; run: PayrollRunDetail["run"] | null; search: string; isAdmin: boolean; canOpenPlan: boolean; canOpenShifts: boolean }) {
  const freshnessProblem = run?.freshness.requiresRecalculation;
  const issueCount = payrollReadinessCheckCount(readiness, Boolean(freshnessProblem));
  ...
  const description = !readiness.canCalculate ? "Нужны план магазина и действующая версия формулы." : freshnessProblem ? "Текущую версию нельзя утверждать или отмечать выплаченной до перерасчета." : !readiness.canApprove ? "Проверка и черновик доступны, но незавершенные проверки пока не позволяют зафиксировать выплату." : "План, формула, классификация, себестоимость и смены прошли автоматические проверки.";
  return <details className={`payroll-readiness payroll-readiness--${tone}`}><summary><span className="payroll-readiness__icon">{tone === "success" ? <ShieldCheck /> : <AlertTriangle />}</span><div><strong>{title}</strong><p>{description}</p></div><b>{readiness.status === "READY" ? "Готово" : formatPayrollCheckCount(issueCount)}</b><i>Проверки</i></summary><div className="payroll-readiness-detail"><div className="payroll-check-grid"><article className={readiness.planPresent ? "ok" : "error"}><span>{readiness.planPresent ? <CheckCircle2 /> : <AlertTriangle />}</span><div><strong>План магазина</strong><small>{readiness.planPresent ? "Найден" : "Не задан"}</small></div>{!readiness.planPresent && (canOpenPlan ? <Link to={{ pathname: "/plan", search }}>Заполнить</Link> : <span>Настроит руководитель с доступом</span>)}</article><article className={readiness.schemePresent ? "ok" : "error"}><span>{readiness.schemePresent ? <CheckCircle2 /> : <AlertTriangle />}</span><div><strong>Версия формулы</strong><small>{readiness.schemePresent ? "Найдена" : "Отсутствует"}</small></div>{!readiness.schemePresent && <span>{isAdmin ? "Откройте настройки" : "Настроит администратор"}</span>}</article><article className={readiness.unmappedItemCount === 0 ? "ok" : "error"}><span>{readiness.unmappedItemCount === 0 ? <CheckCircle2 /> : <AlertTriangle />}</span><div><strong>Классификация</strong><small>{readiness.unmappedItemCount === 0 ? "Все позиции определены" : isAdmin ? `${readiness.unmappedItemCount} позиций без категории` : "Требуется проверка администратором"}</small></div></article><article className={readiness.missingCostItemCount === 0 ? "ok" : "error"}><span>{readiness.missingCostItemCount === 0 ? <CheckCircle2 /> : <AlertTriangle />}</span><div><strong>Себестоимость</strong><small>{readiness.missingCostItemCount === 0 ? "Данные заполнены" : isAdmin ? `${readiness.missingCostItemCount} позиций требуют проверки` : "Требуется проверка администратором"}</small></div></article><article className={readiness.daysWithoutShift === 0 ? "ok" : "error"}><span>{readiness.daysWithoutShift === 0 ? <CheckCircle2 /> : <AlertTriangle />}</span><div><strong>Смены</strong><small>{readiness.daysWithoutShift === 0 ? `${readiness.scheduledDayCount} дней заполнено` : `${readiness.daysWithoutShift} дней с фондом без смены`}</small></div>{readiness.daysWithoutShift > 0 && (canOpenShifts ? <Link to={{ pathname: "/shifts", search }}>Исправить</Link> : <span>Исправит руководитель с доступом</span>)}</article><article className="ok"><span><CheckCircle2 /></span><div><strong>Продажи</strong><small>{readiness.salesDayCount} дней с операциями</small></div></article></div>{run?.freshness.reasons.length ? <section className="payroll-stale-reasons"><strong>Что изменилось после расчета</strong>{run.freshness.reasons.map((reason) => <span key={reason}><RefreshCw />{staleReasonLabel(reason)}</span>)}</section> : null}{isAdmin && readiness.unmappedProducts.length > 0 && <details className="payroll-issue-details"><summary>Товары без зарплатной категории <span>{readiness.unmappedProducts.length}</span></summary>{readiness.unmappedProducts.slice(0, 20).map((product) => <article key={product.productId}><span><strong>{product.productName}</strong><small>{product.suggestedCategoryCode ? `Подсказка: ${payrollCategoryLabel(product.suggestedCategoryCode)}` : "Требуется ручная классификация"}</small></span><b>{product.netQuantity} шт.</b></article>)}</details>}{isAdmin && readiness.missingCosts.length > 0 && <details className="payroll-issue-details"><summary>Позиции без себестоимости <span>{readiness.missingCosts.length}</span></summary>{readiness.missingCosts.slice(0, 20).map((item, index) => <article key={`${item.productId}-${item.payrollDate}-${index}`}><span><strong>{item.productName}</strong><small>{item.payrollDate}, {payrollCategoryLabel(item.payrollCategoryCode)}</small></span><b>{item.quantity} шт.</b></article>)}</details>}{readiness.shiftIssues.length > 0 && <details className="payroll-issue-details"><summary>Дни без состава смены <span>{readiness.shiftIssues.length}</span></summary>{readiness.shiftIssues.map((issue) => <article key={issue.workDate}><span><strong>{issue.workDate}</strong><small>Фонд нельзя надежно распределить</small></span><b>{new Intl.NumberFormat("ru-RU", { style: "currency", currency: "RUB" }).format(issue.fundAmount)}</b></article>)}</details>}</div></details>;
}
...
export function PayrollPage() {
  ...
  const calculateMutation = useMutation({ mutationFn: (reason?: string) => calculatePayroll(storeId, month, reason), onSuccess: onSuccess("Расчет сохранен. Актуальная версия загружена."), onError: onConflict });
  ...
  const paidMutation = useMutation({ mutationFn: () => markPayrollPaid(storeId, latestQuery.data!.run.id, latestQuery.data!.run.version), onSuccess: async (detail) => { await onSuccess("Выплата отмечена. Расчет за месяц завершен.")(detail); await queryClient.invalidateQueries({ queryKey: queryKeys.reportArchive(storeId) }); }, onError: onConflict });
  ...
  const negativePayableCount = detail?.statements.filter((statement) => statement.payableAmount < 0).length ?? 0;
  ...
  const startCalculation = () => {
    if (!detail) calculateMutation.mutate(undefined);
      ...
      if (window.confirm("Пересчитать текущую черновую версию по актуальным источникам?")) calculateMutation.mutate(undefined);
    } else setDialog({ type: "revision" });
  };

  return <div className="payroll-page"><header className="page-heading payroll-heading"><h1>Зарплата</h1>{detail && <div className="payroll-heading-status"><small>Текущая версия</small><strong>№{detail.run.revision}, {statusLabel(detail.run.status)}</strong><span className={`status ${detail.run.freshness.status === "CURRENT" ? "status--success" : "status--warning"}`}>{detail.run.freshness.status === "CURRENT" ? "Актуальна" : "Нужен перерасчет"}</span></div>}</header><Workflow canCalculate={readiness.canCalculate} run={detail?.run ?? null} /><ReadinessBanner readiness={readiness} run={detail?.run ?? null} search={baseSearch} isAdmin={user?.role === "ADMIN"} canOpenPlan={hasUserFeature(user, "PLAN")} canOpenShifts={hasUserFeature(user, "SHIFTS")} />{negativePayableCount > 0 && detail?.run.status === "CALCULATED" && <section className="payroll-risk-warning" role="alert"><AlertTriangle /><div><strong>Есть отрицательные суммы к выплате</strong><p>У {negativePayableCount} сотрудников удержания превышают начисления. Проверьте ведомость перед утверждением.</p></div></section>}{notice && <section className="payroll-notice" role="status"><CheckCircle2 /><span>{notice}</span><button type="button" onClick={() => setNotice(null)} aria-label="Скрыть уведомление"><X /></button></section>}

    <nav className="payroll-tabs" aria-label="Разделы зарплаты"><button className={section === "statement" ? "active" : ""} type="button" aria-current={section === "statement" ? "page" : undefined} onClick={() => selectSection("statement")}><ReceiptText />Ведомость<span>{detail ? `версия №${detail.run.revision}` : "проверка"}</span></button><button className={section === "days" ? "active" : ""} type="button" aria-current={section === "days" ? "page" : undefined} onClick={() => selectSection("days")}><CalendarDays />Дневные фонды<span>{detail?.dailyPools.length ?? preview?.actualScenario.days.length ?? 0} дней</span></button><button className={section === "audit" ? "active" : ""} type="button" aria-current={section === "audit" ? "page" : undefined} onClick={() => selectSection("audit")}><History />История<span>{detail ? `${detail.events.length} событий` : "после расчета"}</span></button></nav>

    {!detail && readiness.canCalculate && previewQuery.isPending ? <PayrollSkeleton /> : section === "statement" ? <PayrollOverview detail={detail} preview={preview} onAddAdjustment={() => setDialog({ type: "adjustment" })} /> : section === "days" ? <PayrollDailyPanel detail={detail} preview={preview} /> : <PayrollAuditPanel storeId={storeId} month={month} timeZone={selectedStore.timezone} detail={detail} onVoidAdjustment={(adjustment) => setDialog({ type: "void", adjustment })} />}

    <aside className="payroll-action-bar"><div><span>{detail ? <FileClock /> : <Calculator />}</span><div><strong>{!detail ? "Проверка без сохранения" : detail.run.freshness.requiresRecalculation ? "Расчет потерял актуальность" : statusLabel(detail.run.status)}</strong><small>{!detail ? "После создания появится версия №1 с полной историей изменений." : detail.run.status === "CALCULATED" ? "Черновик можно пересчитывать и дополнять удержаниями." : "Изменения возможны только через новую версию с причиной."}</small></div></div><section>{detail?.run.status === "CALCULATED" && <button className="button button--ghost" type="button" disabled={busy} onClick={() => setDialog({ type: "adjustment" })}><Plus />Удержание</button>}{detail?.run.status === "CALCULATED" && <button className="button button--ghost" type="button" disabled={!canApprove || busy} title={!canApprove ? "Нужны полный, актуальный расчет и готовность к утверждению" : undefined} onClick={() => setDialog({ type: "approve" })}><ShieldCheck />Утвердить</button>}{detail?.run.status === "APPROVED" && <button className="button button--primary" type="button" disabled={!canPaid || busy} title={!canPaid ? "Перед выплатой требуется перерасчет актуальных источников" : undefined} onClick={() => setDialog({ type: "paid" })}><CircleDollarSign />Отметить выплаченным</button>}{(!detail || detail.run.status === "CALCULATED") && <button className="button button--primary" type="button" disabled={!readiness.canCalculate || busy} onClick={startCalculation}>{detail ? <RefreshCw /> : <Calculator />}{busy && calculateMutation.isPending ? "Выполняем…" : detail?.run.status === "CALCULATED" ? "Пересчитать" : detail ? "Новая версия" : "Создать расчет"}</button>}{detail?.run.status === "APPROVED" && <button className="button button--ghost" type="button" disabled={!readiness.canCalculate || busy} onClick={() => setDialog({ type: "revision" })}><FileClock />Новая версия</button>}{detail?.run.status === "PAID" && <button className="button button--primary" type="button" disabled={!readiness.canCalculate || busy} onClick={() => setDialog({ type: "revision" })}><FileClock />Новая версия</button>}</section></aside>

    {dialog?.type === "adjustment" && <AdjustmentDialog employees={adjustmentEmployees} runVersion={detail!.run.version} busy={adjustmentMutation.isPending || settingsQuery.isPending} error={adjustmentMutation.isError ? payrollErrorMessage(adjustmentMutation.error) : undefined} onClose={closeDialog} onConfirm={(input) => adjustmentMutation.mutate(input)} />}{dialog?.type === "revision" && <ReasonDialog title="Создать новую версию" description="Текущая утвержденная или выплаченная версия останется неизменной. Активные удержания будут перенесены." confirmLabel="Создать версию" busy={calculateMutation.isPending} error={calculateMutation.isError ? payrollErrorMessage(calculateMutation.error) : undefined} onClose={closeDialog} onConfirm={(reason) => calculateMutation.mutate(reason)} />}{dialog?.type === "void" && <ReasonDialog title="Отменить удержание" description={`${dialog.adjustment.employeeName}, ${dialog.adjustment.reason}. Отмена останется в истории и пересчитает ведомость.`} confirmLabel="Отменить удержание" danger busy={voidMutation.isPending} error={voidMutation.isError ? payrollErrorMessage(voidMutation.error) : undefined} onClose={closeDialog} onConfirm={(reason) => voidMutation.mutate({ adjustment: dialog.adjustment, reason })} />}{dialog?.type === "approve" && <ConfirmPayrollDialog title="Утвердить расчет" description={`Версия №${detail!.run.revision} станет неизменяемой. Для следующих изменений потребуется новая версия.${negativePayableCount > 0 ? ` В ведомости ${negativePayableCount} отрицательных сумм к выплате — подтвердите, что вы их проверили.` : ""}`} confirmLabel="Утвердить" danger busy={approveMutation.isPending} error={approveMutation.isError ? payrollErrorMessage(approveMutation.error) : undefined} onClose={closeDialog} onConfirm={() => approveMutation.mutate()} />}{dialog?.type === "paid" && <ConfirmPayrollDialog title="Отметить выплату" description={`Подтвердите, что выплата по версии №${detail!.run.revision} фактически произведена.`} confirmLabel="Отметить выплаченным" danger busy={paidMutation.isPending} error={paidMutation.isError ? payrollErrorMessage(paidMutation.error) : undefined} onClose={closeDialog} onConfirm={() => paidMutation.mutate()} />}
  </div>;
}
