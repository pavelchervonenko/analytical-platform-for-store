...
function PlanResultCard({ title, achieved, actual, target, rate, detail }: { title: string; achieved: boolean; actual: string; target: string; rate: string; detail: string }) {
  return <article className={`payroll-plan-card ${achieved ? "payroll-plan-card--success" : "payroll-plan-card--warning"}`}><header><span>{achieved ? <Check /> : <MinusCircle />}</span><i className={`status ${achieved ? "status--success" : "status--warning"}`}>{achieved ? "Выполнен" : "Не выполнен"}</i></header><small>{title}</small><strong>{actual}</strong><p>Цель: {target}</p><footer><span>Применённая ставка</span><b>{rate}</b></footer><em>{detail}</em></article>;
}
