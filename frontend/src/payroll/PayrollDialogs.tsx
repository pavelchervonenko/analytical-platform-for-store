...
function ModalFrame({ title, description, busy, onClose, children }: { title: string; description: string; busy: boolean; onClose: () => void; children: ReactNode }) {
  ...
  return <div className="payroll-modal-overlay" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) onClose(); }}><section className="payroll-modal" role="dialog" aria-modal="true" aria-labelledby="payroll-dialog-title"><header><div><p className="eyebrow">Зарплата · аудит</p><h2 id="payroll-dialog-title">{title}</h2><p>{description}</p></div><button ref={closeButton} className="icon-button" type="button" disabled={busy} onClick={onClose} aria-label="Закрыть"><X /></button></header>{children}</section></div>;
}
