...
export function PayrollAuditPanel({ storeId, month, timeZone, detail, onVoidAdjustment }: { storeId: string; month: string; timeZone: string; detail: PayrollRunDetail | null; onVoidAdjustment: (adjustment: PayrollAdjustment) => void }) {
  const runsQuery = useQuery({
    queryKey: queryKeys.payrollRuns(storeId, month, 0),
    queryFn: () => getPayrollRuns(storeId, month)
  });
  ...
  if (!detail) return <section className="panel"><div className="panel-empty"><History /><strong>История появится после создания расчета</strong><p>Предпросмотр ничего не сохраняет и не создает версию.</p></div></section>;
  const monthRuns = runsQuery.data?.items ?? [];
