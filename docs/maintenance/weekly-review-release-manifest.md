...
- `frontend/e2e/visual-local.spec.ts` — только `/insights` assertions нового DOM-контракта;
  plan/auth fixture-hunks не включаются.
...
одновременно находились Weekly Review и несвязанные plan/auth fixture-hunks. В кандидат включён
только test-only патч `/insights`: четыре KPI, закрытая по умолчанию структура, одинаковая высота
decision cards на desktop, клавиатурное раскрытие структуры и focus-return detail panel. Расширение
fixture-сценариев допускается в P7-B только отдельным чистым патчем после повторного scope review.
