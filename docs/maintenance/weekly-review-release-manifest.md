...
Проверенный component candidate `codex/weekly-review-rc` на commit `41e5dc8` выбран для
слияния в `codex/store-release-rc`. В Weekly-only конфликтах интеграционный кандидат использует
более позднюю RC-версию; в общих файлах сохраняются обе совместимые семантики.

Граница этого документа по-прежнему описывает только Weekly Review. Общий кандидат дополнительно
содержит LiveSklad recovery, V49–V51, auth/feature permissions, classification и другие UI-пакеты;
их совместная готовность определяется
