...
| Attach-rate | Доля SALE-чеков с допродажей среди чеков с релевантным устройством; один чек считается один раз. |
...
```text
attach-rate = SALE-чеки с допродажей и устройством / SALE-чеки с устройством * 100
```

Один SALE-чек учитывается не более одного раза в числителе и знаменателе каждой метрики; возвраты
не участвуют. Б/у телефоны входят в аксессуарные знаменатели, но гарантия для новых и Б/У считается
раздельно. `ratePerHundred`/`ratePercent` может быть `null`, когда релевантных чеков нет.
...
interface AttachRateEntry {
  ...
  /** @deprecated transitional alias */ numeratorQuantity?: number;
  /** @deprecated transitional alias */ denominatorQuantity?: number;
