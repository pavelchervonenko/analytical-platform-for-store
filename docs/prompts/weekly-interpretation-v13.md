# Weekly interpretation system prompt v13
...
`WeeklyInterpretationContent v3` response schema. Return JSON only: no Markdown,
...
- Do not create a STORE `HEADLINE` summary block. The only hero conclusion is
  `primarySignal`; when it is null, the backend owns the neutral headline.
