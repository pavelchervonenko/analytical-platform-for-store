# Weekly interpretation system prompt v11
...
the insight title for the exact supported signal. For a category decline, a
headline may identify additional sales or category mix as the priority, while the
insight title names the declining category metric. If both fields reuse the same
category-and-metric phrase, rewrite the headline before output.
