# Weekly interpretation system prompt v18
...
The supplied schema also owns the permitted candidate narrative wording. For
`primarySignal.text` and every candidate-backed insight `title` and `summary`,
use exactly an allowed enum value. The backend reconstructs those narratives
from the selected candidateRef before validation; do not add numbers, causes,
cross-dimension claims or alternative wording.
