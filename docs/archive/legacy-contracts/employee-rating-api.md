...
original_content_sha256: 3a624f2af3beb4b0be56480438b5977a99024517c06379a71f8fb9775970571d
required_reviewers:
  - information-architecture
---

> Archived legacy material preserved for provenance. Current replacements: `docs/current/api/employee-rating.md`.
...
Карточка принимает необязательный query-параметр `comparisonMode`:

- `PREVIOUS_PERIOD` — значение по умолчанию; сравнивается непосредственно предшествующий период
  той же продолжительности;
- `PREVIOUS_WEEK` — обе границы выбранного периода сдвигаются на семь дней назад. Поэтому даже
  неполная текущая неделя сравнивается с теми же днями предыдущей недели, а не с ближайшими
  календарными днями перед понедельником.

Frontend отправляет `PREVIOUS_WEEK` только для режима периода «Неделя». Для месяца и произвольного
диапазона используется `PREVIOUS_PERIOD`.
