...
superseded_by:
  - "docs/current/frontend/data-quality-actions.md"
original_content_sha256: 8ab6d9a1ef62cb17f6b79d5ffcde50b5db3c4b059eb402c27a0e3e9b5bd797ab
required_reviewers:
  - information-architecture
---

> Archived legacy material preserved for provenance. Current replacements: `docs/current/frontend/data-quality-actions.md`.

# Frontend: экраны, кнопки и условия действий

Дата проверки по controller, request DTO, security rules и service invariants: 2026-08-24.
...
Представление текущего обзора:

- `PHONES` показывается как вложенная часть `DEVICES`;
- `ACCESSORY` и `SERVICE` показываются как состав `ADDITIONAL_REVENUE`;
- эти пересекающиеся группы нельзя суммировать между собой;
- benchmark карты допродаж — магазин по всем документам;
- основная матрица содержит активных участников рейтинга, а остаток продаж вне roster/без
  сотрудника показывается отдельной контекстной колонкой;
- employee cell сравнивается с магазином: ниже `90%`, уровень `90–110%`, выше `110%`.
...
Подписи не смешивают разные значения: «План месяца», «Выполнено», «Темп на дату»,
«Осталось по плану» и «Нужно в день». Последнее значение динамическое: оно зависит от нового
факта, даты среза и числа оставшихся дней. Таблица будущих дней показывает целевые, а не
...
Runtime flags LLM/Telegram меняются только по соответствующим production runbooks. Frontend не
показывает raw prompts, provider errors, токены, chat IDs или технические reason codes обычному
пользователю. Evidence каждого вывода показывается рядом с ним; общий дублирующий блок «Данные»
не создается, а limited state не заменяется шаблонным текстом.
