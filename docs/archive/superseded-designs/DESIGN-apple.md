...
original_content_sha256: f291f439937fb67828203b1d7913edebe0f62c1a97e74c0f8c7a0d1698dbc8db
required_reviewers:
...
## Store Analytics application profile (2026-08-06)

Этот файл остается референсным анализом Apple, а не буквальным шаблоном маркетингового сайта.
Для рабочей SPA Store Analytics приняты следующие уточнения:

- сохранены системная типографика, спокойная иерархия, белые/жемчужные поверхности, Action Blue
  `#0066cc`, тонкие borders и минимум декоративных теней;
- «жидкое стекло» сохранено на навигационных, sticky и modal-backdrop поверхностях, но не
  используется на каждой карточке; данные должны читаться быстрее декоративного слоя;
- пользовательская копия короткая и русская; backend enum/reason codes, correlation IDs и
  разработческие пояснения не выводятся руководителю магазина;
- фактические application breakpoints: `1280px` для компактной боковой панели, `900px` для
  off-canvas навигации и одноколоночного auth, `820px` для плотных payroll/reports/quality
  workflow, `720px` и `480px` для phone layouts; минимальная поддерживаемая ширина — `320px`;
- touch targets не меньше `44px`, мобильные dialog/sheet используют `100dvh` и safe-area insets,
  широкие таблицы прокручиваются внутри контейнера и не расширяют документ;
- regression viewport: desktop `1440x1000`, tablet `768x1024` с touch, mobile Pixel 7.

Фактический статус реализации и acceptance находятся в `docs/FRONTEND_HANDOFF.md` и
`docs/FRONTEND_ACCEPTANCE.md`. Ниже сохранены исходные дизайн-наблюдения и токены.
